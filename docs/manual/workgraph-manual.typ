...
#table(
  ...
  [*convergence*], [An agent-driven signal (`wg done --converged`) indicating that a loop's iterative work has reached a stable state. Adds a `"converged"` tag to the task. When the source task carries this tag, loop edges do not fire—even if iterations remain and guards are satisfied. Cleared on retry.],
  ...
  [*trace function*], [A parameterized workflow template extracted from completed traces via `wg trace extract`. Captures task structure, dependencies, and loop edges. Instantiated via `wg trace instantiate` to create new task graphs. Stored as YAML in `.workgraph/functions/`.],
